...
Add a console.log() to monitor your worker.

```js title="Updated index.js" {1}
export default {
	async fetch(request, env, ctx) {
		console.log("observe your logs in real-time");
		return new Response("Hello Worker!");
	},
};
```
...
To view real-time logs associated with any deployed Worker, run:

```sh
npx wrangler tail
```
`wrangler tail` surfaces the console.log() output from your live Worker so you can diagnose issues quickly. 
...
- Monitor your worker by [viewing logs, errors, and performance metrics](https://developers.cloudflare.com/workers/observability/) for your deployed Worker.
